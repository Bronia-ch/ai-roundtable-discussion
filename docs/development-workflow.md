...
本阶段按 SDD→DDD→TDD→E2E 顺序完成闭环：先锁定数据库/API 契约，再实现页面状态与事件流，随后用单元测试覆盖命令幂等、写锁和引擎状态机，最后以 3 worker 并发 Playwright 场景验证真实演示。真实 DeepSeek 请求仅由后端读取 `.env`，密钥不进入前端、日志或交付包。交付前清理 `.env`、虚拟环境、依赖缓存和数据库运行文件，并使用 SHA-256 校验交付包。
